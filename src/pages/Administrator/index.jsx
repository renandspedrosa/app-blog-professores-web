...
const columns = [
  {
    name: 'Título',
    selector: (row) =>
      row.title.length > 50 ? `${row.title.substring(0, 20)}...` : row.title,
  },
  {
    name: 'Conteúdo',
    selector: (row) =>
...
const Administrator = () => {
  ...
  const handleRemoveImageWithConfirmation = (postId) => {
    Confirm('', 'Você tem certeza que deseja remover?', () =>
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (
          values.attachmentImg &&
          values.attachmentImg !== postToEdit?.path_img
        ) {
